...
class LocalViewModel: ObservableObject {
    ...
    @Published var location: String {
        didSet {
            UserDefaults.standard.set(location, forKey:"localLocation")
            loadPatientsData()
        }
    }
    ...
    //MARK: - Initialize
    init() {
        location = UserDefaults.standard.string(forKey: "localLocation") ?? ""
        loadPatientsData()
    }
    //MARK: - Functions
    ...
    func getComulPatientsAll() {
        ...
        for item in self.loadedData {
            ...
            comulativePatients.append((item.date, num))
        }
        comulPatientsAll = comulativePatients
    }
